...
from aqt import mw
from aqt.qt import qconnect, debug
    ...
    if current_field is not None:
        for field_index, field_name in enumerate(mw.col.models.fieldNames(editor.note.model())):
            if field_index == current_field and (field_name == config['field_name_expression'] or field_name == config['field_name_meaning']):
                ...
                if search:
                    action = menu.addAction((f"Jisho Automatic Card Generation"))
...
def fill_note_fields_using_jisho(editor: Editor, search):
    ...
    ############### CUSTOM CODE PATCHED HERE ###############
    tags.append(slug_tags)
    ...
    ############### CUSTOM CODE PATCHED HERE ###############    
    try_set_all_tags(editor.note, tags)
...
def normalize_jisho_tag(jtag):
    if jtag.find('wanikani') > -1:
        return 'jisho::wanikani::' + jtag[8:]

    return 'jisho::' + jtag.replace(' ', '_')


def try_set_tag(note, value):
    if len(value) > 0:
        for jisho_tag in value:
            note.tags.append(normalize_jisho_tag(jisho_tag))


def try_set_all_tags(note, all_jisho_tag_info):
    for jisho_tag_info in all_jisho_tag_info:
        try_set_tag(note, jisho_tag_info)
